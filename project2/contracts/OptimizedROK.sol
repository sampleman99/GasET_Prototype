...
contract ROK is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
	uint256 public SALE_NFT = 1907;
	uint256 public GIVEAWAY_NFT = 10;
    ...
    constructor() ERC721P("Royal Outkast's", "ROK") {}
	...
	function mintSaleNFT(uint256 _count) public payable{
        require(_count <= MAX_BY_MINT_IN_TRANSACTION,"Exceeds max mint limit per tnx");
        require(saleEnable, "Sale is not enable");
        require(msg.value >= SALE_PRICE * _count,"Value below price");
        require(SALE_MINTED + _count <= SALE_NFT, "Exceeds max limit");
        require(users[msg.sender].salemint + _count <= MAX_MINT_SALE,"Exceeds max mint limit per wallet");
		uint256 totalSupply = totalSupply();
